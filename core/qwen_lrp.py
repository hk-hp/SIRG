...
import torch
from transformers.models.qwen2 import modeling_qwen2, Qwen2Tokenizer
from lxt.efficient import monkey_patch
import jsonlines
from tqdm import tqdm

path = "model/Qwen2.5-3B"
...
def lrp(prompt, max_token=400):
    ...
    while len(generated_tokens) < max_token:
        ...
        relevance = (input_embeds.grad * input_embeds).float().sum(-1).detach().cpu()[0].tolist()
        ...
        # 检查是否达到终止符 <|endoftext|>
        if next_token == eos_token or next_token == "":
            break
...
if __name__ == '__main__':
    input_file = "data/databricks-dolly-15k_filtered.jsonl"
    output_file = "data/databricks-dolly-15k_output.jsonl"
    index = 0
