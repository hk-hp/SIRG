...
def find_max_decline_numpy(data):
    declines = data[:-1] - data[1:]

    max_decline_idx = np.argmax(declines)
    max_decline = declines[max_decline_idx]
    
    start_idx = max_decline_idx
    end_idx = max_decline_idx + 1
    
    return start_idx, end_idx, max_decline

def process_data(top_k = 3, class_num=2, add_inf = False):
    json_path = 'entities/sent_rel_inf_max_result_' +  LLM_NAME + '.json'
    train_data_file = 'entities/align_train_max_top'+str(top_k) + str(class_num) + '_data_' +  LLM_NAME + '.json'
    test_data_file = 'entities/align_test_max_top' + str(top_k) + str(class_num) + '_data_'+  LLM_NAME + '.json'
   
    ckpt_path = 'save_model/AlignScore-' + MODEL_SIZE + '.ckpt'
    if class_num == 3:
    ...
    context_prompt = 'Context {num}: "{context}"; Contribution Score {num}: {contribution}\n'
    ...
    for example_id, example in tqdm(data.items()): 
        ...
        for index, (node, ratio) in enumerate(zip(graph, globle_ratio)):
            ...
            if add_inf:
                ...
                connect = node / sum(node[1:])
                # connect = node / sum(node) * ratio * 100
                edge_weights.append(connect[1:])
        ...
        for index, edge_weight in enumerate(edge_weights):
            if top_k == 1000: # 动态取大于平均值的
                """edge_indexes = []
                edge_mean = 1 / edge_weight.shape[0]
                edge_rank = np.argsort(edge_weight)[::-1]
                for item in edge_rank:
                    if edge_weight[item] > edge_mean:
                        edge_indexes.append(item)
                edge_indexes = np.array(edge_indexes)"""
                edge_rank = np.sort(edge_weight)[::-1]
                edge_rank_index = np.argsort(edge_weight)[::-1]
                max_decline_begin, max_decline_end, _ = find_max_decline_numpy(edge_rank)
                edge_indexes = edge_rank_index[:max_decline_end]
            else:
            ...
            for i, edge_index in enumerate(edge_indexes):
                if edge_index < len(prompts_sentences):
                    if len(all_sentences[edge_index][3]) == 0:
                        source_sent = all_sentences[edge_index][0]
                    else:
                        source_sent = all_sentences[edge_index][3] + all_sentences[edge_index][0]
                else:
                    source_sent = 'Previous Generate: ' + all_sentences[edge_index][0]
            ...
            claim = response_sentences[index][0]
            
            
            if class_num == 2:
                label = 0 if len(node_lables[index]) == 0 else 1
            elif class_num == 3:
                sentence_label = 1 if len(node_lables[index]) == 0 else 0
    ...
    for key in data_ratio:
        print(key, data_ratio[key] / all_data_num)

if __name__ == "__main__":
    """random.seed(42)
    gen = DataGenerator(list(DATASET_CONFIG.keys()))
    gen.generate()"""
    process_data(top_k=15, class_num=TASK)
