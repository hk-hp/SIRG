...
from alignscore.Parameter import TASK, MODEL_SIZE
...
class Focal_Loss(nn.Module):
    def __init__(self, alpha=None, gamma=2, class_num=2, size_average=True):
...
class BERTAlignModel(pl.LightningModule):
    def __init__(self, model='bert-base-uncased', using_pretrained=True, *args, **kwargs) -> None:
        ...
        if 'muppet' in model:
            ...
            if using_pretrained:
                model = '/home/ /model/roberta-' + MODEL_SIZE
                self.base_model = RobertaModel.from_pretrained(model)
        ...
        self.ce_loss_lm = nn.CrossEntropyLoss(reduction='sum')
        self.ce_loss_2 = nn.CrossEntropyLoss(reduction='sum', weight=torch.tensor([0.1, 0.9], device=self.base_model.device)) # 0.1 0.9
        self.ce_loss_3 = nn.CrossEntropyLoss(reduction='sum', weight=torch.tensor([0.5, 2.0, 0.4], device=self.base_model.device)) # 0.4 0.1 0.5
        # self.ce_loss_3 = Focal_Loss(alpha=torch.tensor([0.1, 1.0, 0.1], device=self.base_model.device), class_num=TASK, size_average=False)
    ...
    def forward(self, batch):
        ...
        if 'mlm_label' in batch.keys(): ### 'mlm_label' and 'align_label' when training
            # ce_loss_fct = nn.CrossEntropyLoss(reduction='sum', weight=self.weight.to(self.base_model.device))

            masked_lm_loss = self.ce_loss_lm(prediction_scores.view(-1, self.base_model.config.vocab_size), batch['mlm_label'].view(-1)) #/ self.con vocabulary
            next_sentence_loss = self.ce_loss_2(seq_relationship_score.view(-1, 2), batch['align_label'].view(-1)) / math.log(2)
            tri_label_loss = self.ce_loss_3(tri_label_score.view(-1, 3), batch['tri_label'].view(-1)) / math.log(3)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            if TASK == 2:
                ...
                seq_relationship_logits = self.softmax(seq_relationship_logits)[:, 1].tolist()
                pred = [int(align_prob>0.5) for align_prob in seq_relationship_logits]

                labels = val_batch['align_label'].tolist()
            elif TASK == 3:
                seq_relationship_logits = output.tri_label_logits
                seq_relationship_logits = self.softmax(seq_relationship_logits)
                seq_relationship = torch.argmax(seq_relationship_logits, dim=1).tolist()
                pred = [int(seq==1) for seq in seq_relationship]

                labels = [int(item==1) for item in val_batch['tri_label'].tolist()]

        return {"pred": pred, 'labels': labels, 'losses': output.all_loss, 'loss_nums': output.loss_nums}#, "preds":preds, "labels":x['labels']}
    ...
    def validation_step_end(self, step_output):
        ...
        return {'total_loss': total_loss, 'pred': step_output['pred'], 'labels': step_output['labels']}

    def validation_epoch_end(self, outputs):
        if not self.is_finetune:
            total_loss = torch.stack(outputs['total_loss']).mean()
            ...
            for each_output in outputs:
                for item in each_output['pred']:
                    all_predictions.extend(item.tolist())
                for item in each_output['labels']:
                    all_labels.extend(item.tolist())
            ...
            self.log("f1", f1, prog_bar=True, sync_dist=True)
