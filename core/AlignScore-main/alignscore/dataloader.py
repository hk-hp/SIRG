...
class DSTDataSet(Dataset):
    ...
    def process_graph3(self, index):
        ...
        return (
            ...
            torch.tensor(-100), # align label, 2 class
            torch.tensor(mlm_labels), # mlm label
            torch.tensor(label), # tri label, 3 class
            torch.tensor(-100.0) # reg label, float
        )

    def __getitem__(self, index):
        if self.dataset[index]['task'] == 'graph_3':
            input_ids, attention_mask, token_type_ids, align_label, mlm_labels, tri_label, reg_label = self.process_graph3(index)

        if self.dataset[index]['task'] == 'graph_2':
            input_ids, attention_mask, token_type_ids, align_label, mlm_labels, tri_label, reg_label = self.process_graph2(index)
...
class DSTDataLoader(LightningDataModule):
    def __init__(self,dataset_config, val_dataset_config=None, sample_mode='seq', model_name='bert-base-uncased', is_finetune=False, need_mlm=True, tokenizer_max_length=512, train_batch_size=32, eval_batch_size=4, num_workers=16, train_eval_split=0.95, **kwargs):
        ...
        assert sample_mode in ['seq', 'proportion', 'graph']
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if self.sample_mode == 'graph':
            self.dataset['train'] = DSTDataSet(dataset=self.raw_dataset[0], model_name=self.model_name, need_mlm=self.need_mlm)
            self.dataset['test'] = DSTDataSet(dataset=self.raw_dataset[1], model_name=self.model_name, need_mlm=self.need_mlm)
            return
    ...
    def init_training_set(self):
        ...
        elif self.sample_mode == 'graph':
            for each_dataset in tqdm(self.dataset_config, desc="Loading data from disk..."):
                with open(self.dataset_config[each_dataset]['data_path'], 'r') as f:
                    example = json.load(f)
                    random.shuffle(example)
                    self.raw_dataset.append(example)
    ...
    def train_dataloader(self):
        if self.sample_mode == 'seq' or self.sample_mode == 'graph':
